!isEmpty(QEXTENSIONSYSTEM_PRI_INCLUDED):error("qextensionsystem.pri already included")
QEXTENSIONSYSTEM_PRI_INCLUDED = 1

exists(config.pri) {
    include(config.pri)
} else {
    error("Does not exists 'config.pri' file, please check documentation for details")
}

QEXTENSIONSYSTEM_VERSION = 3.6.82
QEXTENSIONSYSTEM_COMPAT_VERSION = 3.6.82
...
osx {
    ...
    #TODO: add variable for APP_NAME in the creation of plugins path
    IDE_LIBRARY_PATH = $$IDE_OUTPUT_PATH/$$IDE_LIBRARY_BASENAME/qes
    ...
    win32: \
        ...
        IDE_LIBEXEC_PATH = $$IDE_OUTPUT_PATH/libexec/qes
    !isEqual(IDE_SOURCE_TREE, $$IDE_OUTPUT_PATH):copydata = 1

    LINK_LIBRARY_PATH = $$IDE_BUILD_TREE/$$IDE_LIBRARY_BASENAME/qes
    LINK_PLUGIN_PATH  = $$LINK_LIBRARY_PATH/plugins

    INSTALL_LIBRARY_PATH = $$QTC_PREFIX/$$IDE_LIBRARY_BASENAME/qes
    INSTALL_PLUGIN_PATH  = $$INSTALL_LIBRARY_PATH/plugins
    win32: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/bin
    else: \
        INSTALL_LIBEXEC_PATH = $$QTC_PREFIX/libexec/qes
    INSTALL_DATA_PATH    = $$QTC_PREFIX/share/qes
    INSTALL_DOC_PATH     = $$QTC_PREFIX/share/doc/qes
...
INCLUDEPATH += \
    ...
    $$IDE_SOURCE_TREE/src/utils \
    $$IDE_SOURCE_TREE/src \
    $$IDE_SOURCE_TREE/tools

#message(">>>>>>QTC_PLUGINS_DIRS: " $$QTC_PLUGIN_DIRS)
#message(">>>>>>QES_PLUGINS_DIRS: " $$QES_PLUGIN_DIRS)
...
QTC_PLUGIN_DIRS += $$QES_PLUGINS_DIRS
...
for(ever) {
    ...
    for(dep, QTC_PLUGIN_DEPENDS) {
        ...
        for(dir, QTC_PLUGIN_DIRS) {
            ...
            exists($$dir/$$dep/$${dep}_dependencies.pri) {
                ...
                #message("dependencies_file " $$dependencies_file)
                break()
            }
        }
    ...
    #message("qtc_plugins_deps: " $$QTC_PLUGIN_DEPENDS)
}
